...
###########################################################
    ...
    def limitSol(self):
        ...
        minmod_param_mat=zeros((3,)+U_diff_averfp_r_mat.shape)
        minmod_param_mat[0,:]=U_diff_averfp_r_mat
        minmod_param_mat[1,:]=U_cell_mean_diff_r_mat
        minmod_param_mat[2,:]=U_cell_mean_diff_l_mat
        U_diff_averfp_r_new_mat=minmod(minmod_param_mat)
        minmod_param_mat=zeros((3,)+U_diff_averfp_l_mat.shape)
        minmod_param_mat[0,:]=U_diff_averfp_l_mat
        minmod_param_mat[1,:]=U_cell_mean_diff_r_mat
        minmod_param_mat[2,:]=U_cell_mean_diff_l_mat
        ...
        for ic in idx_arr:
            for isp in range(self.U_sp_mat.shape[0]):
                #  self.U_sp_mat[isp,ic,:]=(self.x_sp_mat[isp,ic]-self.mesh.FluxPts_Mat[0,ic]) * (self.U_fp_mat[0,ic,:]-self.U_fp_mat[1,ic,:]) / (self.x_sp_mat[0,ic]-self.x_sp_mat[-1,ic]) + self.U_fp_mat[0,ic,:] # Biased will blow up.
                #  self.U_sp_mat[isp,ic,:]=(self.x_sp_mat[isp,ic]-self.mesh.CellCenter_Vec[ic]) * (self.U_fp_mat[0,ic,:]-self.U_fp_mat[1,ic,:]) / (self.x_sp_mat[0,ic]-self.x_sp_mat[-1,ic]) + U_cell_mean_mat[ic,:] # P1 MINMOD will oscillate
